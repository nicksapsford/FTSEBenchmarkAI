"""
watchdog_cryptobenchmark.py -- Galahad for FTSEBenchmark (Benchmark Desk).
Launches and supervises main_ftsebenchmark.py: restarts it on unexpected exit,
...
TARGET = BASE_DIR / "main_ftsebenchmark.py"

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s  %(levelname)-7s [Galahad] %(message)s",
                    datefmt="%Y-%m-%d %H:%M:%S")
logging.Formatter.converter = time.gmtime
log = logging.getLogger("FTSEBenchmark.Watchdog")
